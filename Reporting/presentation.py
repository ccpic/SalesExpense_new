...
if __name__ == "__main__":
    ...
    month = 11
    # 准备数据
    df_pre = pd.read_excel("./Reporting/202110.xlsx")  # 作为对比的上个时期的档案数据
    df_post = pd.read_excel("./Reporting/202111.xlsx")  # 作为对比的上个时期的档案数据
    # df_decile = pd.read_excel("decile.xlsx")  # 医院Decile数据文件，用于Decile相关分析的匹配

    df_pre["月份"] = year * 100 + month - 1
    df_post["月份"] = year * 100 + month
    ...
    df_post = df_total[
        (df_total["南北中国"] == bu) & (df_total["月份"] == year * 100 + month)
    ]
    df_total = df_total[df_total["南北中国"] == bu]


    print(df_pre, df_post, df_total)

    pre = Clientfile(df_pre, name="%s%s月" % (bu, month - 1))
    post = Clientfile(df_post, name="%s%s月" % (bu, month))
    total = Clientfile(df_total, name=bu)

    print("Data Ready")

    # 创建ppt
    prs = Presentation("./Reporting/template.pptx")
    w = prs.slide_width
    h = prs.slide_height
    ...
    data_list = [
        [
            df_post.shape[0],
            df_post.shape[0] - df_pre.shape[0],
            df_post.shape[0] / df_pre.shape[0] - 1,
        ],
        [
    ...
    for i in range(4):
        ...
        p.text = "(%s, %s)" % (
            "{:+,.0f}".format(data_list[i][1]),
            "{:+.1%}".format(data_list[i][2]),
        )
        p.alignment = PP_ALIGN.CENTER
        if data_list[i][1] > 0:
    ...
    for col in columns_list:
        add_img_slide(
            ...
            0,
            {
                "img": post.plot_barline_dist(
                    index="大区", columns=col, values=None, perc=False
                ),
                "top": TOP,
                "left": LEFT_ONE_IMAGE,
                "width": WIDTH_ONE_IMAGE,
        ...
        add_img_slide(
            ...
            {
                "img": post.plot_barline_dist(
                    index="大区", columns=col, values=None, perc=True
                ),
                "top": TOP,
                "left": LEFT_ONE_IMAGE,
                "width": WIDTH_ONE_IMAGE,
        ...
        add_img_slide(
            ...
            {
                "img": post.plot_barline_dist(
                    index="大区", columns=col, values=None, perc=False, pre=pre
                ),
                "top": TOP,
                "left": LEFT_ONE_IMAGE,
                "width": WIDTH_ONE_IMAGE,
    ...
    add_img_slide(
        ...
        {
            "img": post.plot_hist_dist(
                pivoted=False, show_kde=True, show_tiles=True, bins=100, tiles=10
            ),
            "top": TOP,
            "left": LEFT_ONE_IMAGE,
            "width": WIDTH_ONE_IMAGE,
    ...
    # 保存ppt文件
    prs.save("./Reporting/presentation.pptx")
