...
class Staff(models.Model):
    ...
    desendants = models.CharField(max_length=999)

    def set_desendants(self, x):
        self.desendants = json.dumps(x)

    def get_desendants(self):
        return json.loads(self.desendants)
...
class Client(SoftDeletableModel):
    bu = models.CharField(max_length=10, choices=BU_CHOICES, verbose_name="南北东中国")
    ...
    hp_level = models.CharField(
        max_length=4, choices=HPLEVEL_CHOICES, verbose_name="医院级别"
    )
    ...
    def monthly_patients(self):
        return round(
            self.consulting_times * self.patients_half_day * self.target_prop / 100, 0
        )
    ...
    def updated_status(self):
        if self.is_deleted is True:
            updated_obj = (
                Client.objects.all_with_deleted()
                .filter(
